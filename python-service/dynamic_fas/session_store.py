...
def reconcile_state(
    ...
    for question in request.questions:
        ...
        # A non-empty answer in the visible form is authoritative. This lets the
        # assistant detect updates without ever writing application data itself.
        if field.status == "pending_update":
            should_adopt_current_answer = (
                current_answer and current_answer == field.pending_value
            )
        else:
            should_adopt_current_answer = current_answer and (
                field.status in {"missing", "existing"}
                or current_answer == field.value
            )
...
def update_navigation(state: DynamicAssistantState) -> None:
    pending_update = next(
        (
            ...
            if (field := state.questions[key]).status == "pending_update"
        ),
        None,
    )
    ...
    state.pending_question_id = next(
        (
            field.question_id
            for key in state.question_order
            if (field := state.questions[key]).is_required and not field.value
        ),
        None,
    )
...
def get_suggested_fields(state: DynamicAssistantState) -> dict[str, str]:
    return {
        key: field.value
        for key, field in state.questions.items()
        if field.status == "suggested" and field.value
    }
...
def get_progress(state: DynamicAssistantState) -> DynamicProgress:
    required_fields = [
        state.questions[key]
        for key in state.question_order
        if state.questions[key].is_required
    ]
    ...
    return DynamicProgress(
        ...
        required_question_ids=[field.question_id for field in required_fields],
    )
