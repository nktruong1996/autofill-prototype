...
def _pending_update_field(
    state: DynamicAssistantState,
) -> DynamicQuestionState | None:
...
def handle_chat(request: DynamicChatRequest) -> DynamicChatResponse:
    ...
    if not request.questions:
        return _build_response(
            request,
            state,
            "This FAS does not currently have any additional questions.",
        )

    pending_field = _pending_update_field(state)

    # Confirmations are handled deterministically before message routing.
    if pending_field is not None:
        if is_positive_confirmation(request.message):
            ...
            return _build_response(
                request,
                state,
                "Reply “Yes” to update the answer or “No” to keep the current answer.",
            )
    ...
    if route.category != "FORM_FILLING":
        return _build_response(
            ...
            state,
            route.reply
            or "I can only help complete the questions in the FAS application.",
        )
    ...
    return _build_response(
        request,
        state,
        _next_question_reply(state, extracted_any=extracted_any),
    )
